...
# Pie: Outcomes ###
def tourny_all_pie(out_com, tourny_pick, year_pick):
        pie_labels = list(out_com.keys())
        pie_values = list(out_com.values())
...
#Bar: Outcomes
def tourny_all_bar(out_com, tourny_pick, year_pick): 
        bar_x = dict(sorted(out_com.items(), key=lambda item: item[1], reverse=True))
        ...
        all_outcomes_bar = px.bar(
            x=bar_labels,
            y=bar_values,
            title="Outcomes For " + str(tourny_pick) + " in " + str(year_pick),
            color=bar_labels,
...
# Bubbles: Outcomes 
def tourny_all_bubbles(out_com, tourny_pick, year_pick): 
        bubble_x_outcomes = dict(sorted(out_com.items(), key=lambda item:item[1], reverse=True))
        ...
        for value in (bubble_outcome_values_percent):
                bubble_outcome_values_percent_format.append(f"{value:2.1%}")
        ...
        title = "Outcomes For " + str(tourny_pick)
                                          + " in "
                                          + str(year_pick),                                  
        ...
        all_outcomes_bubbles.update_layout(title_font_color="#bf365b")
...
# Waffle: Outcomes ###
def tourny_all_waffle(out_com, tourny_pick, year_pick):
        # plot setup
        ...
        waffle_x_outcomes = dict(sorted(out_com.items(), key=lambda item:item[1], reverse=True))
        ...
        colour = ['#3366CC', '#DC3912', '#FF9900', '#109618', '#990099', 
        ...
        fig = plt.figure(FigureClass=Waffle, figsize=(10,10), rows=5, columns = 20, 
                title={
                        'label': "Outcomes For " + str(tourny_pick) + " in " + str(year_pick),
                        'color': '#bf365b',
                        'loc': 'left',
                        'fontdict': {
                                'fontsize': 20,
