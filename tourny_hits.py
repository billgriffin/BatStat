...
# Hits for Tournament_Summary

# Pie 
def tourny_pie_plot(hits, tourny_pick, year_pick): 
    labels_pie = list(hits.keys())
    sizes_pie = list(hits.values())
    
    tourny_all_hits_pie = px.pie(
        values=sizes_pie, 
        names=labels_pie, 
        title=("Hits By Location For " + str(tourny_pick) + " in " + str(year_pick)),
        color_discrete_sequence=px.colors.qualitative.G10,
        hole=.25
    )
    tourny_all_hits_pie.update_traces(
        textposition='inside', 
        textinfo='percent+label',
        textfont_color="black")
    tourny_all_hits_pie.update_layout(
        title_font_color="#bf365b", 
        legend=dict(title='Location')
    )
    return tourny_all_hits_pie

# Bar
def tourny_bar_plot(hits, tourny_pick, year_pick): 
    ...
    bar_values_percent = []
    bar_values_percent_format = []    
    bar_values_percent = [((value/bar_values_sum)) for value in bar_values]
    for value in (bar_values_percent):
        bar_values_percent_format.append(f"{value:2.1%}")
    
    tourny_all_hits_bar = px.bar(
        x=bar_labels,
        y=bar_values,
        text_auto=True,
        title=f"Hits By Location For {tourny_pick} in {year_pick}",
        color=bar_labels,
    ...
    tourny_all_hits_bar.update_layout(
        title_font_color="#bf365b",
        legend=dict(title='Location'), 
    ...
    return tourny_all_hits_bar

# Bubbles
def tourny_bubbles_plot(hits, tourny_pick, year_pick):
    ...
    bubbles = pd.DataFrame(
        list(zip(bubble_labels, bubble_values, bubble_values_percent_format)),
    ...
    bubbles['X'] = list_x
    label = [f"{i}<br>{j}<br>{k}%" for i, j, k in zip(bubbles.location, bubbles.frequency, bubbles.percent)]
        
    tourny_all_hits_bubbles = px.scatter(
        bubbles,
        x='X',
        y='Y',
        color='location',
        ...
        text=label,
        size_max=90,
        title=f"Hits For {tourny_pick} in {year_pick}"
    ) 
    
    tourny_all_hits_bubbles.update_layout(
        width=900,
        height=420,
    ...
    tourny_all_hits_bubbles.update_traces(textposition='top center')
    tourny_all_hits_bubbles.update_layout(title_font_color="#bf365b",
                                          font_color= "black")
    tourny_all_hits_bubbles.update_xaxes(showgrid=False, zeroline=False, visible=False)
    tourny_all_hits_bubbles.update_yaxes(showgrid=False, zeroline=False, visible=False)
    return tourny_all_hits_bubbles

# Waffle: Hits ###
def tourny_waffles_plot(hits, tourny_pick, year_pick):
    # plot setup
    ...
    colour = ['#3366CC', '#DC3912', '#FF9900', '#109618', '#990099', '#0099C6', '#DD4477', '#66AA00', '#B82E2E', '#316395']
    ...
    fig = plt.figure(FigureClass=Waffle, figsize=(10,10), rows=5, columns = 20, 
                         title={
                    'label': "Hits By Location For " + str(tourny_pick) + " in " + str(year_pick),
                        'color': '#bf365b',
                        'loc': 'left',
                        'fontdict': {
                            'fontsize': 20,
